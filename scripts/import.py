...
def main():
    ...
    for project_id in json_data['metadata']['products']:
        ...
        if data is None:
            # Add missing project
            print 'Added project:', current_project['name']
            cursor.execute(
            ...
            if data[0] != current_project['name']:
                print 'Updating project name from "{0}" to "{1}"'.format(current_project['name'], data[0])
                cursor.execute(
    ...
    print 'Added projects:', added_projects
